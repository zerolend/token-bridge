...
const main = async function () {
    ...
    const endpoint = await ethers.getContractAt('IEndpointV2', await oft.endpoint())

    const remoteConnections = connections.filter((c) => c.network !== hre.network.name)
    ...
    for (let index = 0; index < remoteConnections.length; index++) {
        ...
        if (peer.toLowerCase() != remoteOft.toLowerCase()) {
            const data = await oft.populateTransaction.setPeer(remote.peerId, remoteOft)
